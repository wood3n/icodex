...
在浏览器的网页中，一段 JS 的代码始终是以`script`的形式被加载，JS 没有 main 函数这种起始执行位置，它是按代码顺序被解析执行的。以 V8 为例，JS 代码经过词法分析，语法分析等编译步骤后会生成 AST，然后 V8 的解释器 Ignition 开始对 AST 进行遍历，这个过程就是解释执行的过程，可以使用执行上下文栈来形象的比喻这个执行的过程：
