...
JS 的执行过程是从上往下，从左往右进行的，

### 等于号

#### ===

> [`x === y`](https://tc39.es/ecma262/#sec-strict-equality-comparison)
...
这个操作符是在 ES2020 正式列入规范的，目前 V8 引擎已经支持了，其为 JS 判断值是否为`undefined`提供了极大的便利，过去经常使用上面的逻辑运算符`||`以及`&&`来进行`null`和`undefined`的判断，例如：
...
这里面有`0`的情况是比较特殊的，有时候后端返回的业务状态值中使用正整数的枚举值会从`0`开始，前端根据状态来处理，如果使用逻辑运算符判断就会出错。
