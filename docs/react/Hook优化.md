...
但是函数组件就不行，函数组件不是作为构造函数去执行，而是**以函数的形式直接被调用执行**，有时候想通过 hooks 去模拟一些`class`组件的生命周期，但是 hooks 的表现并不像完整的生命周期那样，例如`useEffect`，`useEffect`甚至比`useState`还要常用。
...
同理可以使用`useCallback`优化组件内部调用函数的执行时机，但是并不是所有的函数都需要优化，
