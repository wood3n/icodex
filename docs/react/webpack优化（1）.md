...
拆分出的 chunk 不会被以`<script>`的形式插入到 HTML 中，也就是首次加载并不会去请求这个 chunk，而当页面中用到的时候才会去请求加载。这里简单以一个例子来看一下，我们编写一个计算工资的组件，组件很简单，只包含一个`button`和一个`p`标签，其内部实现点击`button`动态导入计算函数的`math.js`模块，然后`setState`去改变页面显示结果。
